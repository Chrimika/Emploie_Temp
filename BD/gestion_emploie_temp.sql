...
--
-- Hôte : 127.0.0.1
-- Généré le : ven. 16 fév. 2024 à 23:35
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertPlanning` (IN `p_id_enseignant` INT, IN `p_heure_debut` TIME, IN `p_jour` VARCHAR(50))   BEGIN
    INSERT INTO dispenser(id_enseignant, heure_debut, jour)
    VALUES (p_id_enseignant, p_heure_debut, p_jour);
END$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `CountProgrammeCours` (`p_id_enseignant` INT) RETURNS INT(11)  BEGIN
    DECLARE count INT;
...
INSERT INTO `dispenser` (`id_enseignant`, `heure_debut`, `jour`) VALUES
(46, '12:00:00', 'Jeudi'),
(46, '07:30:00', 'Mardi');
...
(46, 'TCHINDA MBA', 'aXog8deUvn', 'CHRISTIAN MIKA', 'mbachristian58@gmail.com', 3);
